      ...
      >
        <Card className="bg-white/95">
          <CardHeader>
            ...
            <CardDescription>Choose how this component should behave in your game</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid md:grid-cols-2 gap-6">
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <h3 className="text-lg font-semibold">Option A: {component.variants.A.name}</h3>
                  <Badge variant="outline" className="bg-purple-100">
                    A
                  </Badge>
                </div>
                <p className="text-gray-600 mb-4">{component.variants.A.description}</p>
                <div className="space-y-2">
                  {/* Show key features */}
                  <div className="text-sm text-gray-500">
                    <span className="font-medium">Key Features:</span>
                    <ul className="mt-1 list-disc list-inside">
                      {component.variants.A.name.includes('Floaty') && (
                        <li>Variable jump height</li>
                      )}
                      {component.variants.A.name.includes('Real-time') && (
                        <li>Fast-paced action</li>
                      )}
                      {component.variants.A.name.includes('Confetti') && (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <h3 className="text-lg font-semibold">Option B: {component.variants.B.name}</h3>
                  <Badge variant="outline" className="bg-pink-100">
                    B
                  </Badge>
                </div>
                <p className="text-gray-600 mb-4">{component.variants.B.description}</p>
                <div className="space-y-2">
                  {/* Show key features */}
                  <div className="text-sm text-gray-500">
                    <span className="font-medium">Key Features:</span>
                    <ul className="mt-1 list-disc list-inside">
                      {component.variants.B.name.includes('Realistic') && (
                        <li>Predictable physics</li>
                      )}
                      {component.variants.B.name.includes('Turn-based') && (
